...
{
    int isOpen = Serial_Open("/dev/ttyUSB0");
    if (isOpen ==0) isOpen = Serial_Open("/dev/ttyUSB1");
    if (isOpen ==0) isOpen = Serial_Open("/dev/ttyUSB2");
    if (isOpen ==0) isOpen = Serial_Open("/dev/ttyUSB3");
    if (isOpen ==0) isOpen = Serial_Open("/dev/ttyUSB4");
...
void SerialPort::SerialWaterFish(int& enemycolor,int& targetnum,bool& predict,DetectMode& detectMode,float& gyro_pitch,float& gyro_yaw)
{
    ...
    int detectMode_int=(int)recv_buff[start+4];
    switch (detectMode_int) {
    case 0:
        detectMode = DetectMode::Armor;
        break;
    case 1:
        detectMode = DetectMode::Minbuff;
        break;
    case 2:
        detectMode = DetectMode::Bigbuff;
        break;
    default:
        break;
    }
