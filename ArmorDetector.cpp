...
ArmorDetector::ArmorDetector()
{
    ...
    loadSVM("/home/cod-2th/Desktop/file_SVM/Infantry1_0123467_0623.xml");
    ...
    for (int i = 0; i < lights.size()-1 ; i++)
    {
        for (int j = i + 1; j < lights.size(); j++) //just ensure every two lights be matched once 从左至右，每个灯条与其他灯条一次匹配判断
        {
            ...
            if (armor.isSuitableArmor()) //when the armor we constructed just now is a suitable one,set extra information of armor 如果是合适的装甲板，则设置其他装甲板信息
            {
                ...
                {
                    if (armor.armorNum == 1 || armor.armorNum == 7) armor.type = ArmorType::BIG_ARMOR;
...
void ArmorDetector::setNumScore(const int& armorNum, const int& targetNum, float& armorScore)
{
    if (targetNum == 0)
    {
        if (armorNum == 1) armorScore += 100000;
        else if (armorNum == 2) armorScore += 1000;
        else if (armorNum == 3) armorScore += 5000;
        else if (armorNum == 4) armorScore += 5000;
        else if (armorNum == 5) armorScore += 5000;
        else if (armorNum == 6) armorScore += 4000;
    }
...
{
    //cout<<a_score <<" aaa=========comp and ============bbb  " endl; //score
    ...
    cout<<a_score <<" aaa=====================bbb  " <<b_score<<endl; //score
 ...
 */
void ArmorDetector::getTargetInfo(vector<Point2f>& lightVertices, Point2f& centerPoint, ArmorType& type,bool& flag_Num)
{
    lightVertices.clear();
